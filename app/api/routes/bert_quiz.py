"""
app/api/routes/bert_quiz.py
Endpoint pentru generarea quizurilor fill-in-the-blank cu modelul BERT local
"""
...
router = APIRouter(prefix="/bert-quiz", tags=["BERT Quiz"])


class BertQuizRequest(BaseModel):
    material_id: int
...
class BertQuizOut(BaseModel):
    ...
    generated_by: str = "bert_local"


@router.post("/generate", response_model=BertQuizOut, status_code=201)
async def generate_bert_quiz(
    body: BertQuizRequest,
    ...
    try:
        ...
        questions_data = generate_fillblank_questions(text, body.num_questions)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Eroare la generarea BERT: {str(e)}")
    ...
    quiz_title = body.title or f"Quiz ML — {material.title}"
