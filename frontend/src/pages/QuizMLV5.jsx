// frontend/src/pages/QuizMLV5.jsx
import { useState, useEffect, useRef } from "react";
import { getToken } from "../api/auth";
import {
  Flame, AlertTriangle, Info, FileText,
  ChevronDown, X, Loader2, Globe, Search, Zap, CheckCircle
} from "lucide-react";
...
async function generateBertQuizV5(payload) {
  const res = await fetch(`${API_URL}/bert-quiz-v5/generate`, {
    method: "POST",
...
function BtnGroup({ label, options, value, onChange, accent = "#8b5cf6" }) {
  return (
    <div>
    ...
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(o => !o)}
        ...
        style={{
          ...
          border: `1px solid ${open ? "rgba(139,92,246,0.45)" : "var(--card-border)"}`,
        }}
        ...
        <div className="flex items-center gap-3 min-w-0">
          <FileText size={15} style={{ color: selected ? "#8b5cf6" : "var(--text-sub)" }} className="flex-shrink-0" />
          ...
          <div className="max-h-64 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              ...
              <button key={m.id} onClick={() => handleSelect(m)}
                ...
                style={{ backgroundColor: selected?.id === m.id ? "rgba(139,92,246,0.08)" : "transparent" }}
                ...
                <FileText size={14} style={{ color: selected?.id === m.id ? "#8b5cf6" : "var(--text-sub)" }} className="flex-shrink-0" />
...
export default function QuizMLV5({ onStartQuiz }) {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const quiz = await generateBertQuizV5({ material_id: selected.id, num_questions: numQuestions });
      ...
      <div className="w-full max-w-2xl space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center"
              style={{ backgroundColor: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.28)" }}>
              <Flame size={20} style={{ color: "#8b5cf6" }} />
            </div>
          ...
          <p className="text-sm" style={{ color: "var(--text-sub)" }}>
            Completează spațiile libere — varianta mai dificilă. Variantele greșite sunt mai greu de eliminat.
          </p>
        </div>

        {/* Ce e diferit față de Quiz Cloze standard */}
        <div className="rounded-2xl p-5 flex gap-4"
          style={{ backgroundColor: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.2)" }}>
          <Flame size={20} className="flex-shrink-0 mt-0.5" style={{ color: "#8b5cf6" }} />
          <div>
            <p className="text-sm font-bold mb-1" style={{ color: "#a78bfa" }}>
              De ce e mai greu decât Quiz Cloze standard?
            </p>
            <p className="text-sm leading-relaxed" style={{ color: "var(--text)" }}>
              Variantele greșite sunt semantic mai apropiate de răspunsul corect —
              nu le poți elimina intuitiv, trebuie să{" "}
              <span style={{ color: "#a78bfa" }}>înțelegi contextul</span>, nu doar să-l recunoști.
              Recomandat când vrei să testezi dacă stăpânești cu adevărat materia,
              nu doar dacă ai văzut-o. Dacă ești la început,
              încearcă mai întâi{" "}
              <span style={{ color: "#60a5fa", fontWeight: "600" }}>Quiz Cloze standard</span>.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5 flex gap-4"
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed" style={{ color: "var(--text)" }}>
              Ca și varianta standard, rulează complet pe server{" "}
              <span style={{ color: "#fcd34d" }}>fără API extern</span>.
              Dificultatea mai mare vine din modul în care sunt generați distractorii —
              nu înseamnă că toate întrebările sunt perfecte.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6"
          ...
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-bold" style={{ color: "#a78bfa" }}>Cum funcționează?</p>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {[
              { step: "1", label: "Extrage propoziții", desc: "complete din PDF" },
              { step: "2", label: "Alege un cuvânt",    desc: "cheie din propoziție" },
              { step: "3", label: "Variante dificile",  desc: "semantic similare cu răspunsul" },
            ].map(s => (
        ...
        {generating && (
          <div className="flex items-center gap-3 rounded-xl px-5 py-4"
            ...
            <Loader2 size={17} className="animate-spin flex-shrink-0" style={{ color: "#8b5cf6" }} />
            <div>
        ...
        <button onClick={handleGenerate} disabled={!selected || generating || !!pendingQuiz}
          ...
          style={{
            backgroundColor: selected && !generating && !pendingQuiz ? "#8b5cf6" : "var(--card)",
            ...
            boxShadow:        selected && !generating && !pendingQuiz ? "0 0 20px rgba(139,92,246,0.25)" : "none",
          }}
          onMouseEnter={e => selected && !generating && !pendingQuiz && (e.currentTarget.style.backgroundColor = "#7c3aed")}
          onMouseLeave={e => selected && !generating && !pendingQuiz && (e.currentTarget.style.backgroundColor = "#8b5cf6")}>
          {generating ? (
            <><Loader2 size={15} className="animate-spin" />Se generează...</>
          ) : selected ? (
            <><Zap size={15} />Generează {numQuestions} întrebări</>
          ) : (
